...
### 使用教程模块提案记录 (2026-03-30 UTC)
- 已完成“是否只做一个网页”的方案评估，结论是：
  - 如果当前目标只是让首批内部用户能快速上手，做一个独立的“使用教程”页面就够，优先解决“第一次进来不知道怎么操作”的问题。
  - 如果目标是长期对外、多人协作或后续功能会持续增长，则不应停留在单页静态说明，而应逐步演进成“帮助中心 + 上下文引导 + 常见问题”的组合。
- 建议本模板按“两阶段”落地：
  - 第一阶段：前端新增独立路由页，放入侧栏导航，内容包括“3 步上手”“任务流说明”“状态说明”“AI 配置说明”“常见失败与处理办法”。
  - 第二阶段：在任务工作台、AI 配置页和登录桥接页补充上下文帮助入口，例如空状态提示、字段旁说明、失败态指引和深链接跳转到教程页对应锚点。
- 当前最佳实践口径：
  - 教程内容应与真实界面强绑定，围绕“用户要完成什么任务”来组织，而不是写成纯概念介绍。
  - 教程页必须覆盖关键异常路径，尤其是会话失效、AI 超时重试、最终失败、`review_required` 四类状态。
  - 信息结构应优先采用“快速开始 + 详细说明 + FAQ”，避免单块长文案。
...
- 已将任务列表、任务详情与帮助中心中的状态码说明改为中文表达，并把默认 AI 配置草稿名称同步调整为中文。
- 本轮本地验证命令：
  - `cd apps/service-frontend && npm run typecheck`
  - `cd apps/service-frontend && npm run build`

### 使用教程模块实施决策记录 (2026-03-30 UTC)
- 已评估当前模板复杂度，结论是该需求适合“先写简短计划、同一轮直接实现”，不需要拆成多次独立开发。
- 判断依据：
  - 当前前端路由较少，页面集中在 `Dashboard / Workbench / Profiles / Login` 四个入口，教程信息架构清晰。
  - 教程页、上下文帮助和 FAQ 目前都属于前端静态信息层，不依赖后端新增接口或数据库结构变更。
  - 现有样式系统已经具备 `panel / banner / empty-state / advanced-box` 等可复用容器，落地成本可控。
- 本轮实现建议边界：
  - 新增独立 `GuidePage` 路由和侧栏入口。
  - 在 `Workbench / Profiles / Login / Dashboard` 增加轻量上下文帮助区块或深链接入口。
  - 在教程页中整合 FAQ、状态解释、排错指引和锚点导航。
  - 同步补充开发计划或 PRD 中对“帮助与引导模块”的说明。
- 暂不纳入本轮的扩展项：
  - 富文本 CMS
  - 远程可配置帮助内容
  - 多语言切换
  - 埋点分析
  - 首次登录分步 tour
- 如果本轮执行顺利，默认应一次提交完成；只有在后续要求“可运营化帮助系统”时，才值得拆出第二阶段专题开发。

### 使用教程模块实现记录 (2026-03-30 UTC)
- 已在前端新增独立帮助中心页面：
  - `apps/service-frontend/src/pages/GuidePage.vue`
  - `apps/service-frontend/src/router/index.ts`
  - `apps/service-frontend/src/App.vue`
- 已完成“教程页 + 上下文帮助 + FAQ”三层结构：
  - 教程页集中承载快速开始、任务流说明、状态判断、AI 配置策略、会话说明、故障排查和 FAQ。
  - `Dashboard / Workbench / Profiles / Login` 四个页面均已补充场景内帮助区块或深链接入口。
  - `TaskCreateCard` 已补齐字段级说明文案，降低首次填写成本。
- 已补充样式支持：
  - 新增 `guide-layout / guide-nav / context-grid / status-grid / field-hint / quick-actions / mini-stats` 等布局和帮助样式。
  - 已兼容移动端收敛为单列布局。
- 已同步更新产品文档：
  - `docs/PRD/service_prd.md` 新增“使用教程与帮助引导”要求。
- 已完成本轮验证：
  - `cd apps/service-frontend && npm run typecheck`
  - `cd apps/service-frontend && npm run lint`
  - `cd apps/service-frontend && npm run build`
