...
from apps.backend.schemas.ops import HealthCheckItem, HealthResponse
...
async def build_health_response(
    ...
    checks["queue"] = HealthCheckItem(status=queue_status, detail=f"worker status: {metrics['status']}")
    ...
    response = HealthResponse(
        ...
        uptime_sec=uptime,
        checks=checks,
        metrics={k: int(v) for k, v in metrics.items() if k != "status"},
